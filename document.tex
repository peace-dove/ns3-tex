...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.7]{picture/p.png}
	\caption{丢弃概率对吞吐量的影响}
	\label{fig:p}
\end{figure}
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{picture/cwTcp.png}
	\caption{TCP窗口大小与时间关系}
	\label{fig:cWTCP}
\end{figure}

在程序刚刚开始运行时，$WestWood$、$Veno$爬升的很快，$Vegas$爬升程度只有上述算法的一半。
...
经过以上实验，我们可以看到在标准环境下，$WestWood$凭借其算法特性窗口大小浮动很大，而$Vegas$和$Veno$则相对稳健，其中$Veno$更是能够在丢包事件中实现接受字节数小幅度减少。当然我们必须要注意到，当前网络拓扑结构只是网络环境的一个特例，并非能够涵盖所有网络拓扑结构。
